...
class AnswerEngine {
    ...
    static let hashes: [String] = ["a","b","c","d","e","f"]
    static var hashIndex: Int = Int.random(in: 0..<6)
    ...
    static func storeAnswersInCache() {
        ...
        UserDefaults.standard.set(hashIndex, forKey: "hashIndex")
    }
    ...
    static func retrieveFromCache(_ completionHandler: @escaping () -> ()) {
        ...
        hashIndex = UserDefaults.standard.integer(forKey: "hashIndex")
    ...
    static func reset() {
        ...
        UserDefaults.standard.set(nil, forKey: "hashIndex")
    }
    ...
    static private func retrieveMoreAnswers(_ completionHandler: @escaping () -> ()) {
        retrieveAnswerPoolSizeAndDate { s, d in
            ...
            if (size > 100) {
                 ...
                 */
                // first grab newest
                let topQuery = self.db.collection("users").whereField("hash", isEqualTo: hashes[hashIndex]).order(by: "answers."+date+".winPercentage", descending: true).limit(to: 20)
