...
class UserEngine {
    ...
    static var rankings: Set<Answer>?
    static var following: Set<User>?
    
    /*
     Retrieves "users" document associated with the current user
     */
    static func getUserDocument(_ completionHandler: @escaping () -> ()) {
        if db == nil {
            db = Firestore.firestore()
        }
        
        let userQuery = self.db.collection("users").whereField("id", isEqualTo: user.firebaseID).limit(to: 1)
        userQuery.getDocuments() { (qs, error) in
            ...
            guard snapshot.documents.count > 0 else {
                setNewUserDocument(completionHandler)
                return
            }
            for document in snapshot.documents {
                ...
                user.answers = processAnswersFromDoc(docData, document.documentID)
                user.orderAnswers()
                break
            }
            completionHandler()
    ...
    static private func setNewUserDocument(_ completionHandler: @escaping () -> ()) {
        var ref: DocumentReference? = nil
        ref = db.collection("users").addDocument(data: [
            "username": "",
            "id": user.firebaseID,
            "hash": AnswerEngine.hashes[Int.random(in: 0..<6)],
            "following": [],
        ]) { err in
            if let docID = ref?.documentID {
                UserEngine.user.docID = docID
            }
            completionHandler()
        }
    }
     ...
     */
    static private func processAnswersFromDoc(_ doc: [String: Any]?, _ docID: String) -> [String: Answer] {
        let result = doc?["answers"] as? [String: Any]
        let author = doc?["username"] as? String ?? "..."
        var answerMap: [String: Answer] = [:]

        guard let answers = result else {
            return answerMap
        }
        for (date, _map) in answers {
            guard let map = _map as? [AnyHashable: Any] else {
                continue
            }
            let answerString = map["answer"] as? String ?? "..."
            let promptString = map["prompt"] as? String ?? "..."
            let w1 = map["winPercentage"] as? Float ?? 0.0
            let w2 = Float(map["winPercentage"] as? Double ?? 0.0)
            let winPercentage = max(w1, w2)
